...
class CoreNLPTokenizer(Tokenizer):
    ...
    def _launch(self):
        ...
        options = ','.join(['untokenizable=noneDelete',
                            'invertible=true'])
        cmd = ['java -Xmx2g -cp "../../stanford-corenlp-4.4.0/*"', 
#                '-cp',
               '"%s"' % self.classpath,
               'edu.stanford.nlp.pipeline.StanfordCoreNLP','-props arabic', '-annotators',
        ...
        self.corenlp = pexpect.spawn('/bin/bash', maxread=100000, timeout=606)
